...
// Traitement de la suppression d'un sujet si le formulaire de confirmation est soumis
if (isset($_POST['supprimer']) && isset($_POST['id_sujet'])) {
    // (int) : cast en entier pour éviter toute injection via ce champ
    $id_sujet = (int)$_POST['id_sujet'];

    // Supprime d'abord toutes les réponses liées au sujet (contrainte de clé étrangère)
    $sqlDelRep = "DELETE FROM reponse WHERE ref_sujet = :id";
    $qDelRep   = $connexion->prepare($sqlDelRep);
    $qDelRep->execute(['id' => $id_sujet]);

    // Puis supprime le sujet lui-même
    $sqlDel = "DELETE FROM sujet WHERE id_sujet = :id";
    $qDel   = $connexion->prepare($sqlDel);
    $qDel->execute(['id' => $id_sujet]);

    // Redirige avec un paramètre GET pour afficher un message de succès
...
// Requête avec WHERE 1=1 pour ajouter des filtres dynamiquement
// La sous-requête COUNT(*) donne le nombre de réponses par sujet
...
$params = [];
if ($recherche !== '') {
    $sql .= " AND (sujet.titre LIKE :recherche OR inscrit.pseudo LIKE :recherche)";
    $params['recherche'] = '%'.$recherche.'%';
}
if ($categorie !== '') {
    $sql .= " AND sujet.categorie_sujet = :categorie";
...
// Récupère les catégories distinctes pour le filtre (PDO::FETCH_COLUMN = tableau simple)
...
<head>
    ...
    <?php if ($daltonisme_session !== 'aucun' || $dyslexie_session): ?>
        <style id="accessibilite-overrides">
            ...
            .btn-danger:hover,.btn-danger:active { background-color:<?= $p['danger_dark'] ?> !important; }
            <?php endif; ?>
            ...
            *,*::before,*::after{font-family:'OpenDyslexic',Arial,sans-serif !important;}
            body{line-height:1.8 !important;letter-spacing:.05em !important;background-color:#fdfaf3 !important;}
...
<div class="container my-5">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <!-- Pluralisation dynamique du badge de comptage -->
        <span class="badge bg-danger fs-6"><?= count($sujets) ?> sujet<?= count($sujets) > 1 ? 's' : '' ?></span>
    </div>

    <!-- Message de confirmation affiché après suppression réussie (paramètre GET 'supprime') -->
    ...
    <!-- Formulaire de filtre en GET : n'affecte que l'URL, pas les données -->
                ...
                <div class="col-md-4">
                    ...
                    <select name="categorie" class="form-select">
                        ...
                        <?php foreach ($categories as $cat): ?>
                            <!-- Marque l'option sélectionnée si elle correspond au filtre actif -->
                        ...
                        <?php foreach ($sujets as $sujet): ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <!-- data-id et data-titre : transmettent les données du sujet au modal JS via dataset -->
        ...
        <div class="modal-content border-danger border-2">
            ...
            <div class="modal-footer">
                ...
                <form method="POST" action="forum_admin.php">
                    <!-- Champ caché rempli dynamiquement par le JS avec l'ID du sujet ciblé -->
...
<script>
    ...
    // Événement déclenché avant l'affichage du modal Bootstrap
    // e.relatedTarget : le bouton qui a déclenché l'ouverture du modal
    // dataset.id / dataset.titre : les attributs data-* définis sur le bouton dans la boucle PHP
